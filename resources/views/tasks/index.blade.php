                        ...
                        @forelse ($tasks as $task)
                            <li class="list-group-item p-3">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-5">
                                        <a href="{{ route('tasks.show', $task->id) }}" class="fw-bold text-decoration-none {{ $task->completed ? 'text-decoration-line-through text-muted' : '' }}">
                                            {{ $task->name }}
                                        </a>
                                    ...
                                    <div class="col-md-3 text-end">
                                        ...
                                        <form action="{{ route('tasks.destroy', $task->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('¿Eliminar tarea?')">Eliminar</button>
                                        </form>
                                    </div>
